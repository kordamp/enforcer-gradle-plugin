 ...
 */
@CompileStatic
class RequireGradleProperty extends AbstractPropertyEnforcerRule {
    final Property<String> property
    final Property<Boolean> after

    @Inject
    RequireGradleProperty(ObjectFactory objects) throws EnforcerRuleException {
    ...
    protected void doExecute(EnforcerContext context) throws EnforcerRuleException {
        ...
        switch (context.enforcerPhase) {
            ...
            case BEFORE_PROJECT:
                if (!after.get()) {
                    enforceValue(context.project.findProperty(property.get()))
                }
                break
            case AFTER_PROJECT:
            case AFTER_PROJECTS:
